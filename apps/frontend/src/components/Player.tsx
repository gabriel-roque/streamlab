...
type Props = {
  ...
  preview?: string;
  audioTracks?: string[];
  subtitles?: Array<{ label: string; language: string; src: string }>;
...
export function Player({
  ...
  useEffect(() => {
    ...
    const updateMetrics = () => {
      ...
      const next = {
        ...metrics,
        buffer: `${Math.max(0, buffered).toFixed(1)} s`,
    ...
    video.addEventListener("playing", () => {
      if (startedAt.current) {
        ...
        const next = { ...metrics, startup: `${startup} s` };
        setMetrics(next);
        onMetrics?.(next);
        startedAt.current = 0;
      }
  ...
  function seek(value: number) {
    ...
    setProgress(value);
    onTelemetry?.("seek", { position: video.currentTime });
  }
  ...
  return (
    <div className="player-shell">
      <div className="player-stage">
        ...
        {error && (
          <div className="player-error">
            ...
            <span>
              Use the API playback URL or try again when the source is online.
            </span>
          </div>
        )}
        {!playing && !error && (
        ...
        <div className="player-topline">
          <span className="player-live">
            <span />
            {protocol === "HLS" ? "HLS / ADAPTIVE" : "PROGRESSIVE MP4"}
          </span>
        ...
        <div className="player-controls">
          {preview && previewPosition !== null && (
            ...
            >
              <img src={preview} alt="" />
              <span>{formatTime((previewPosition / 100) * duration)}</span>
            </div>
          )}
          <input
            className="seek"
          ...
          <div className="controls-row">
            ...
            <label className="select-control" aria-label="Quality">
              <span>quality</span>
              <select
                value={quality}
            ...
            {audioTracks?.length ? (
              <label className="select-control" aria-label="Audio">
                <span>audio</span>
                <select
                  defaultValue="default"
                  onChange={(event) => {
                    const index = audioTracks.indexOf(event.target.value);
                    if (hlsRef.current && index >= 0)
                      hlsRef.current.audioTrack = index;
                    onTelemetry?.("audio_change", {
      ...
      <div className="player-readout">
        <span>
          <b>BUFFER</b> {metrics.buffer}
        </span>
        <span>
          <b>ABR</b> {quality === "auto" ? "active" : "locked"}
        </span>
        <span>
          <b>CAPTIONS</b> {subtitles?.length ? "available" : "none"}
        </span>
        <span>
          <b>CODEC</b> H.264
        </span>
